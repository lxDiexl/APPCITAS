    ...
    public function store(Request $request)
    {
        ...
        for ($i = 0; $i < 7; ++$i) {
            if ($morning_start[$i] > $morning_end[$i]) {
                $errors[] = 'Inconsistencia en el internvalo de las horas del turno de la mañana del día ' . $this->days[$i] . '.';
            }
            if ($afternoon_start[$i] > $afternoon_end[$i]) {
                $errors[] = 'Inconsistencia en el internvalo de las horas del turno de la tarde del día ' . $this->days[$i] . '.';
            }
